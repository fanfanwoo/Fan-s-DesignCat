...
import { AnalysisState, DesignContext, ChatSessionData } from './types';
...
const determineTheme = (
  ...
  if (userPreference === 'day') return { theme: 'day', reason: "User explicitly prefers day mode." };
  if (userPreference === 'night') return { theme: 'night', reason: "User explicitly prefers night mode." };
  if (systemTheme === 'light') return { theme: 'day', reason: "System theme maps to day mode." };
  if (systemTheme === 'dark') return { theme: 'night', reason: "System theme maps to night mode." };
  if (localHour >= 7 && localHour <= 18) return { theme: 'day', reason: "Time is between 7-18." };
  return { theme: 'night', reason: "Time is outside 7-18." };
};
...
function App() {
  ...
  useEffect(() => {
    ...
    if (decision.theme === 'night') root.classList.add('dark');
    else root.classList.add('light');
    
    if (userPreference) localStorage.setItem('user_theme_pref', userPreference);
    else localStorage.removeItem('user_theme_pref');
  ...
  const handleAnalyze = async (image: string | null, context: DesignContext) => {
    ...
    try {
      const { text, scores } = await analyzeDesign(image, context, decision.theme);
      
      // Initialize chat data
      const initialChatData: ChatSessionData = {
        history: [{ role: 'model', text: 'I have analyzed your design. You can ask me to "visualize improvements" or "create a new version" to see code.' }],
        lastContext: context,
        lastImage: image
      };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
             ...
             <button 
               ...
               title={`Current Mode: ${decision.theme}`}
             >
               {getThemeIcon()}
             </button>
          </div>
      ...
      <main className="flex-1 w-full px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        {state.status === 'error' && (
           <div className="max-w-md mx-auto text-center p-8 bg-white dark:bg-slate-800 rounded-xl border border-red-100 dark:border-red-900/50 shadow-lg animate-fade-in transition-colors duration-300">
              ...
              <button onClick={handleReset} className="px-6 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-900 dark:text-white rounded-lg transition-colors font-medium">
                Try Again
              </button>
           </div>
        )}
